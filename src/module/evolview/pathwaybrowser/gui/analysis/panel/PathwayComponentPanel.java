...
public class PathwayComponentPanel extends AbstractTsvBasedAnalysisPanel {

    private static final Logger log = LoggerFactory.getLogger(PathwayComponentPanel.class);
    private final String inputFile;

    private Map<String, List<String>> name2RowData = Collections.emptyMap();

    public PathwayComponentPanel(PathwayBrowserController controller, String tsvFilePath) {
        super(controller);
    ...
    private void displayRowForNode(String nodeName) {
        ...
        if (rowData == null) {
            removeAll();
            add(buildInfoPanel("No pathway component data found for: " + nodeName));
            revalidate();
            repaint();
            return;
        }
