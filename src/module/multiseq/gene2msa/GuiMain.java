...
//		jideTabbedPane.setTabShape(JideTabbedPane.SHAPE_ROUNDED_VSNET);
		...
		JPanel importDataDialog = voicm4TreeLeafInforObtainer.generateImportDataDialogGUI();
		importDataDialog.setBorder(null);

		jideTabbedPane.setFont(defaultTitleFont);
		// 先不用这个Tab因为感觉GUI很好用
//		jideTabbedPane.addTab("VOICM import panel", null, importDataDialog,
//				"Setting parameters by the VOICM mechinism.");
	...
	private JComponent getCommonGUIsettingPanel() {
		...
		JComponent centerPanel = BuildTreeAllRelatedParametersConfigGUI.getCenterPanel4gene2msa();
		return centerPanel;
	}
	...
	public String[] getFeatureNames() {
		return new String[] {"Obtain the alignment from website"};
	}
	...
	public IInformation getModuleInfo() {
		IInformation iInformation = new IInformation() {
			...
			public String getSummaryOfResults() {
				return "The gene to multiple sequence alignment functionality is powered by the eGPS software.";
			}
