...
//				}
		...
		JComponent centerPanel = BuildTreeAllRelatedParametersConfigGUI.getCenterPanel4buildTreeFromMAF();
	...
	protected void runTheProgram(String concat) {
		...
		boolean isGzip = inputFileParameterPanel.isGZIP();
		String fileSuffix = "maf";
		if (isGzip) {
			fileSuffix = "maf.gz";
		}
		...
		for (File file2 : listFiles) {
			if (file2.getName().endsWith(fileSuffix)) {
				inputFiles.add(file2);	
			}
		}
		
		PLMAF2PhyloTreeBenchMode plWeb2ObtainAlignment = new PLMAF2PhyloTreeBenchMode(inputFiles);
	...
	public String[] getFeatureNames() {
		return new String[] { "Contruct phylogenetic tree from MAF." };
	}
	...
	public IInformation getModuleInfo() {
		IInformation iInformation = new IInformation() {
			...
			public String getWhatDataInvoked() {
				return "The MAF files is imported by user.";
			}

			@Override
			public String getSummaryOfResults() {
				return "The MAF files to phylogenetic tree functionality is powered by the eGPS software.";
			}
