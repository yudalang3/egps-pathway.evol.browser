...
向用户显示消息时，统一使用 `SwingDialog`。它内部基于 `ModernDialog` + HTML 3.2 + `JEditorPane` + `JScrollPane`。
...
如果用户点击了 “Yes” 返回 `true`，否则返回 `false`。
...
- `showConfirmDialog` 是 **阻塞** 的，会等待用户响应
- Builder 的 `.show()` 是 **阻塞** 的；`.build()` 只返回 dialog 对象，交给调用方自行控制
